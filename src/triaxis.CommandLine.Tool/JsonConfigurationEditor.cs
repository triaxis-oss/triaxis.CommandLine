...
internal static class JsonConfigurationEditor
{
    ...
    public static string Apply(string original, IEnumerable<KeyValuePair<string, string?>> changes)
    {
        ...
        try
        {
            ...
            int pos = 0;
            Node root = ParseValue(ref reader, bytes, ref pos);
            string trailing = Slice(bytes, pos, bytes.Length);
    ...
    private static Node ParseValue(ref Utf8JsonReader r, byte[] b, ref int pos)
    {
        Tok tok = Take(ref r, b, ref pos);
        switch (r.TokenType)
        {
            case JsonTokenType.StartObject:
                ...
                while (r.Read())
                {
                    ...
                    {
                        obj.Close = Take(ref r, b, ref pos);
                        return obj;
                    }
                    string key = r.GetString() ?? "";
                    var name = Take(ref r, b, ref pos);
                    var member = new Member { Name = name, Key = key };
                    r.Read();
                    member.Value = ParseValue(ref r, b, ref pos);
            ...
            case JsonTokenType.StartArray:
                ...
                while (r.Read())
                {
                    ...
                    {
                        arr.Close = Take(ref r, b, ref pos);
                        return arr;
                    }
                    var elem = new Elem();
                    var v = ParseValue(ref r, b, ref pos);
    ...
    private static Tok Take(ref Utf8JsonReader r, byte[] b, ref int pos)
    {
        ...
        var tok = new Tok { Lead = Slice(b, pos, start), Text = Slice(b, start, start + len) };
        pos = start + len;
        return tok;
    }
